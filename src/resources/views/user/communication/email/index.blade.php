                        ...
                        @forelse($logs as $log)
                            <tr>
                                ...
                                <div class="d-flex align-items-center gap-1">
                                    <button data-log-id="{{ $log->id }}" data-log-status="{{ $log->status }}" data-log-status-message="{{ translate("Status: ").ucfirst(strtolower(\App\Enums\CommunicationStatusEnum::getValue($log->status))) }}" data-subject="{{ $log->message['subject'] }}"  data-updated-at="{{ Carbon\Carbon::parse($log->updated_at)->toDayDateTimeString() }}" class="icon-btn btn-ghost btn-sm info-soft circle update-log">
...
<script>
	"use strict";
        ...
        $('.update-log').on('click', function() {
            ...
            modal.find('.log-view').attr('href', '{{ route("user.communication.email.view", ":id") }}'.replace(':id', logId));
            modal.modal('show');
        });
</script>
@endpush
