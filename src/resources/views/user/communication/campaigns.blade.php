                    ...
                    <div class="col-xxl-8 col-lg-9 offset-xxl-1">
                        <div class="filter-action">
                            <select data-placeholder="{{translate('Select A Delivery Status')}}" class="form-select select2-search" name="status" aria-label="{{translate('Select A Delivery Status')}}">
                                <option value=""></option>
                                @foreach(\App\Enums\CampaignStatusEnum::toArray() as $key => $value)
                ...
                <table>
                    ...
                    <tbody>
                        @forelse($campaigns as $campaign)
                            @php
                                $type = strtolower(\App\Enums\ServiceType::getValue($campaign->type));
                            @endphp
                            <tr>
                                <td>
                                    <div class="form-check">
                                ...
                                <td data-label="{{ translate('Message Logs')}}">
                                    <a href="{{route("user.communication.$type.index", ['campaign_id' => $campaign->id])}}" class="badge badge--primary p-2">
                                        <span class="i-badge info-solid pill">
                                            {{ translate('View all').' ('.$campaign->communicationlog->count().') ' }} <i class="ri-eye-line ms-1"></i>
                                        </span>
                                    </a>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex flex-column gap-1 align-items-start ">
                                        ...
                                        <span class="text-dark fw-medium">{{ $campaign->repeat_time == 0 ? translate("Executes: Once") : translate("Gets delivered in every ").$campaign->repeat_time. " ". ucfirst(strtolower(\App\Enums\CampaignRepeatEnum::getValue($campaign->repeat_format)) ) }}</span>
                                    </div>
                                </td>
                                <td>
                                    @if(checkCredit($user, strtolower(\App\Enums\ServiceType::getValue($campaign->type))) && $campaign->status == \App\Enums\CampaignStatusEnum::DEACTIVE->value)
                                        <div class="switch-wrapper checkbox-data">
                                           
                                            <input {{ $campaign->status == \App\Enums\CampaignStatusEnum::ACTIVE->value ? 'checked' : '' }}
                                                    type="checkbox"
                                                    class="switch-input statusUpdate"
                                                    data-id="{{ $campaign->id }}"
                                                    data-column="status"
                                                    data-value="{{ $campaign->status }}"
                                                    data-route="{{route('user.communication.campaign.status.update')}}"
                                                    id="{{ 'status_'.$campaign->id }}"
                                                    name="is_default"/>
                                            <label for="{{ 'status_'.$campaign->id }}" class="toggle">
                                                <span></span>
                                            </label>
                                        </div>
                                    @else 
                                        @php echo campaign_status($campaign->status) @endphp
                                    @endif
                                </td>
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <a class="icon-btn btn-ghost btn-sm success-soft circle" href="{{ route("user.communication.$type.campaign.edit", $campaign->id) }}">
...
<div class="modal fade actionModal" id="bulkAction" tabindex="-1" aria-labelledby="bulkAction" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered ">
        ...
        <form action="{{route('user.communication.sms.campaign.bulk')}}" method="POST" enctype="multipart/form-data">
            @csrf
...
<div class="modal fade actionModal" id="deleteCampaign" tabindex="-1" aria-labelledby="deleteCampaign" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered ">
        ...
        <form action="{{route('user.communication.sms.campaign.delete')}}" method="POST">
            @csrf
            <div class="modal-body">
                <input type="hidden" name="id" value="">
                <div class="action-message">
