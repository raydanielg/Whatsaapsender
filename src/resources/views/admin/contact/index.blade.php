...
<main class="main-body">
    <div class="container-fluid px-0 main-content">
      ...
      <div class="table-filter mb-4">
        <form action="{{route(Route::currentRouteName(), ['group_id' => $groupId])}}" class="filter-form">
                ...
                <div class="col-xxl-8 col-lg-9 offset-xxl-1">
                    <div class="filter-action">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <a class="filter-action-btn bg-danger text-white" href="{{route(Route::currentRouteName(), ['group_id' => $groupId])}}">
        ...
        <div class="card-header">
            ...
            <div class="card-header-right">
                <div class="d-flex gap-3 align-item-center">
                    ...
                    <a class="i-btn btn--info btn--sm export-csv">
                        <i class="ri-add-fill fs-16"></i> {{ translate("Export Contacts") }}
                    </a>
                    ...
                    <tr>
                        ...
                        <td> 
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <i id="verify-icon-{{ $contact->uid }}" class="ri-loop-left-line cursor-pointer" onclick="verifyEmailAndUpdate('{{ route('admin.verify.email') }}', '{{ $contact->uid }}', '{{ $contact->email_contact }}')"></i>
                                <span class="i-badge {{$contact->email_verification == 'verified' ? 'success' : 'danger'}}-soft pill">
                                    @if($contact->email_verification == 'verified') 
                                        <i data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{translate($contact->email_verification)}}" class="ri-check-double-line"></i>
                                    @else
                                        <i data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{translate($contact->email_verification)}}" class="ri-close-line"></i>
                                    @endif
                                </span>
                        ...
                        <td data-label="{{ translate('Status')}}">
                            <div class="switch-wrapper checkbox-data">
                                <input {{ $contact->status == \App\Enums\Common\Status::ACTIVE->value ? 'checked' : '' }}
                                        ...
                                        data-route="{{route('admin.contact.status.update')}}"
                            ...
                            @endphp
                                ...
                                <button class="icon-btn btn-ghost btn-sm success-soft circle update-contact" data-bs-toggle="modal" data-bs-target="#updateContact" href="javascript:void(0)"
                                    ...
                                    <i class="ri-edit-line"></i></button>
                                        ...
                                        type="button"
                                        data-url        = "{{route('admin.contact.destroy', ['uid' => $contact->uid])}}"
                                        data-bs-toggle="modal"
                ...
                <div class="modal-body modal-lg-custom-height">
                    <div class="row g-4">
                        ...
                        <div class="col-lg-12">
                            ...
                            @if(site_settings('email_contact_verification') == \App\Enums\Common\Status::ACTIVE->value)
                                <div class="input-group">
                                    <input type="text" 
...
<div class="modal fade actionModal" id="deleteContact" tabindex="-1" aria-labelledby="deleteContact" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered ">
        ...
        <form action="{{route('admin.contact.destroy')}}" method="POST" id="singleContactDelete">
            @csrf
...
<script type="text/javascript">
    ...
    function updateEmailVerification(uid, email_verification) {
        $.ajax({
            url: '{{ route("admin.contact.update.email.verification") }}',
            method: 'POST',
            data: {
                uid: uid,
    ...
    <script>
        (function($){
            ...
            $('.update-contact').on('click', function() {
                ...
                var activeAttributesCount = 0;
                $.each(filtered_meta_data, function(key, meta) {
                    if (meta.status === '{{ \App\Enums\Common\Status::ACTIVE->value }}') {
                ...
                // Iterate over filtered_meta_data
                $.each(filtered_meta_data, function(key, meta) {
                    if (meta.status !== '{{ \App\Enums\Common\Status::ACTIVE->value }}') return;

                    currentIndex++;
                    var isLastItem = currentIndex === activeAttributesCount;
                    var colClass = 'col-lg-6'; // Default class

                    // Adjust column class for the last item
                    if (isLastItem) {
                        colClass = (activeAttributesCount % 2 === 1) ? 'col-12' : 'col-lg-6'; // Odd: col-12, Even: col-lg-6
                    }
