                    ...
                    <tbody>
                        @forelse($logs as $log)
                            <tr>
                                ...
                                <td>
                                    <p> 
                                        {{ array_key_exists('gateway', $log->meta_data) ? ucfirst($log->meta_data['gateway']) : (@$log->whatsappGateway ? translate("WhatsApp Cloud API") : translate("N\A"))  }}
                                        <span data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{ array_key_exists('gateway_name', $log->meta_data) ? $log->meta_data['gateway_name'] : (@$log->whatsappGateway->name ? $log->whatsappGateway->name : translate("N\A")) }}">
                                ...
                                <div class="d-flex align-items-center gap-1">
                                    ...
                                    <button data-log-id="{{ $log->id }}" data-log-status="{{ $log->status }}" data-log-status-message="{{ translate("Status: ").ucfirst(strtolower(\App\Enums\CommunicationStatusEnum::keyVal($log->status))) }}" data-message="{{ $log->message["message_body"] == [] || gettype($log->message["message_body"]) == 'array'  ? translate("Template Message") : $log->message["message_body"]}}" data-updated-at="{{ Carbon\Carbon::parse($log->updated_at)->toDayDateTimeString() }}" class="icon-btn btn-ghost btn-sm info-soft circle update-log">
                                        <i class="ri-eye-line"></i>
                                    </button>
                                    <button data-log-id="{{ $log->id }}" class="icon-btn btn-ghost btn-sm danger-soft circle text-danger delete-whatsapp-log" type="button" data-bs-toggle="modal" data-bs-target="#deleteWhatsappLog">
...
<div class="modal fade actionModal" id="bulkAction" tabindex="-1" aria-labelledby="bulkAction" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered ">
        ...
        <form action="{{route('admin.communication.bulk', ['type' => \App\Enums\ServiceType::WHATSAPP->value])}}" method="POST" enctype="multipart/form-data">
            @csrf
