        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-header-right">
                    <a class="i-btn btn--info btn--sm me-2"
                        href="{{ route('admin.system.setting', ['type' => 'webhook']) }}">
                        <i class="ri-server-line fs-16"></i> {{ translate('Configure Webhook') }}
                    </a>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td data-label="{{ translate('Templates') }}">
                                    <a href="{{ route('admin.template.whatsapp.index', ['id' => $item->id]) }}"
                                        class="badge badge--primary p-2">
                                        <span class="i-badge info-solid pill">
                                            {{ translate('View all') . ' (' . count($item->template) . ') ' }} <i
                                ...
                                <td data-label={{ translate('Option') }}>
                                    <div class="d-flex align-items-center gap-1">
                                        <button
                                            ...
                                            type="button" data-id="{{ $item->id }}" data-name="{{ $item->name }}"
                                            data-credentials="{{ json_encode($item->credentials) }}" data-bs-toggle="modal"
                                        ...
                                        <button
                                            ...
                                            type="button" data-item-id="{{ $item->id }}" data-bs-toggle="modal"
...
<div class="modal fade" id="addWhatsappBusinessAccount" tabindex="-1" aria-labelledby="addWhatsappBusinessAccount" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered ">
        <div class="modal-content">
            <form action="{{route('admin.gateway.whatsapp.cloud.api.save')}}" method="POST">
                @csrf
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                $('.sync').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.template.refresh') }}',
                        type: 'GET',
                        data: {
                            itemId: itemId
                        },
